...
- `std.crypto.random` no longer exists as a namespace. UUID generation on the WASM target (where there is no OS entropy source) calls `fill_random_bytes`, an `extern` function imported from the JS host (`edge/worker.js`) and backed by `crypto.getRandomValues`. This preserves the CSPRNG guarantee on the edge; see ADR-04 in `docs/adr/decisions.md` for the full rationale.
...
The `Command`, `DomainEvent`, and `AuditEvent` envelopes are the vocabulary of the runtime. They carry `tenant_id`, `user_id`, and `timestamp` on every record as required structural members, not optional fields. Multi-tenancy is not an afterthought here; it is load-bearing.
